...
class GPTQ:
    ...
    def fasterquant(
        ...
        H = self.H
        print(f"--- PYTORCH DEBUG ---")
        print(f"Initial W sum: {torch.sum(W).item():.6f}")
        print(f"Initial H sum: {torch.sum(H).item():.6f}")
        del self.H
        dead = torch.diag(H) == 0
        H[dead, dead] = 1
        W[:, dead] = 0
        ...
        for i1 in range(0, self.columns, blocksize):
            ...
            for i in range(count):
                ...
                if i1 == 0 and i == 0: # Print only for the very first column
                    ...
                    print(f"First column q sum: {torch.sum(q).item():.6f}")
                    print(f"First column err sum: {torch.sum(err1).item():.6f}")
        ...
        if isinstance(self.layer, transformers.Conv1D):
            Q = Q.t()
        print(f"Final Q sum: {torch.sum(Q).item():.6f}")
        print(f"--- END PYTORCH DEBUG ---")
